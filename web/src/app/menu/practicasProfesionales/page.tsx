...
export default function Home() {
  ...
  useEffect(() => {
    const fetchPracticas = async () => {
      try {
        const response = await fetch(`http://localhost:3000/api/getPracticaProfesional?page=${page}`);
        if (!response.ok) {
  ...
  const handleNavigation = (id: number) => {
    router.push(`/menu/practicasProfesionales/${id}`);
  };
  ...
  useEffect(() => {
    ...
    router.replace(`/menu/practicasProfesionales?page=${page}`);
  }, [page, router]);
